...
export default async function VocabDetail({ params }: PageProps) {
  ...
  if (!currentVocab) {
    return (
      <div className="flex-grow flex flex-col items-center justify-center p-6 text-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12 text-slate-300 dark:text-slate-600 mb-3 mx-auto">
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
        </svg>
        <h3 className="font-bold text-slate-700 dark:text-slate-300 text-lg">단어를 찾을 수 없습니다</h3>
      </div>
    );
  }
  ...
  return <VocabDetailClient currentVocab={currentVocab} prevVocab={prevVocab} nextVocab={nextVocab} vocabIndex={vocabIndex} totalCount={vocabData.length} />;
}
