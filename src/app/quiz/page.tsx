...
import { buildQuizOptions, getCategories, getCategoryLabel, pickDistractors, shuffle, type VocabItem } from 'src/lib/study';

interface Question {
  correctItem: VocabItem;
  type: 'keywordToSummary' | 'summaryToKeyword' | 'blank' | 'ox' | 'compare';
  prompt: string;
  options: {
    id: string;
    text: string;
...
export default function Quiz() {
  ...
  const generateQuiz = () => {
    ...
    const generatedQuestions: Question[] = quizItems.map((correctItem) => {
      // 퀴즈 문제 유형 랜덤 결정
      const types: Question['type'][] = ['keywordToSummary', 'summaryToKeyword', 'blank', 'ox', 'compare'];
      ...
      if (type === 'compare') {
        const pair = distractors[0];
        return {
          correctItem,
          type,
          prompt: `${correctItem.keyword}와 ${pair.keyword} 중 다음 설명에 더 가까운 용어는?`,
          options: buildQuizOptions(correctItem, [pair], (item) => item.keyword),
        };
      }
